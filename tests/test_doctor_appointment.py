...
class TestDoctorAppointment:
    ...
    def click_consult(self):
        ...
        """
        try:
            self.helper.click(AppointmentPageLocators.hospital_visit)
            expected_text_1 = self.reader.read_data(sheet_name="verify_data", row_number=5, column_name="E")
            self.helper.verify(AppointmentPageLocators.consult, expected_text_1)
